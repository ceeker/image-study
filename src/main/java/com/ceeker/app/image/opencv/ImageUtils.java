...
public class ImageUtils {
    ...
    public BufferedImage matToBufImg( String imgFile ) {
        Mat grayMat = Imgcodecs.imread(imgFile);
        byte[] data1 = new byte[grayMat.rows() * grayMat.cols() * (int) (grayMat.elemSize())];
        grayMat.get(0, 0, data1);
        BufferedImage bufferedImage = new BufferedImage(grayMat.cols(),
                grayMat.rows(), BufferedImage.TYPE_BYTE_GRAY);
        bufferedImage.getRaster().setDataElements(0, 0, grayMat.cols(), grayMat.rows(), data1);
